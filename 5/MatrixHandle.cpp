...
int parseNumberFromString(const char const* digitBeginPtr, int* numeralCountOutput) {
	...
	while ((digitBeginPtr[numeralCount] != ' ') && (digitBeginPtr[numeralCount] != 0)) {
		if ((digitBeginPtr[numeralCount] > '9') || (digitBeginPtr[numeralCount] < '0')) {
			return -1;
		}
...
//sets concrete element if matrix is initialized
void ArrayOfMatrix::setMatrixElement(const char const* nameBeginPtr) {
	...
	if ((nameBeginPtr[digitIndex - 1] == 0) || (nameBeginPtr[digitIndex] == 0)) {
	...
	if (x == -1) {
		std::cout << "inappropriate number entered. operation canceled\n\n";
		return;
	}
	if (mtxArr[currentActiveArray][matrixIndex]->getSize() - 1 < x) {
		std::cout << "entered index is out of bounds of matrix. operation canceled\n\n";
		return;
	}
	...
	if (mtxArr[currentActiveArray][matrixIndex]->getSize() - 1 < y) {
		std::cout << "entered index is out of bounds of matrix. operation canceled\n\n";
		return;
	}
	
	if (mtxArr[currentActiveArray][matrixIndex]->isInitialized() == false) {
		std::cout << "matrix is not initialized. you should initialize it with \"fill (name)\"\n\n";
		return;
	}
	...
	mtxArr[currentActiveArray][matrixIndex]->setElement(x, y, input);
}

//sets negative matrix elements to zero
void ArrayOfMatrix::negateNegative(const char const* nameBeginPtr) {
	int matrixIndex = getMatrixIndex(nameBeginPtr);
	...
	int size = mtxArr[currentActiveArray][matrixIndex]->getSize();

	for (int i = 0; i < size; ++i) {
		for (int j = 0; j < size; ++j) {
			if ((*mtxArr[currentActiveArray][matrixIndex])(i, j) < 0) {
				mtxArr[currentActiveArray][matrixIndex]->setElement(i, j, 0);
			}
		}
	}
}
